...
class Model:
    def __init__(self, in_shape):
        ...
        with tf.name_scope("decoder"):
            ...
            #self.kl_loss = -0.5 * tf.reduce_sum(1 +        log_sigma    - tf.square(mu)          - tf.exp(log_sigma) , axis=1)
            #latend_loss  = -0.5 * tf.reduce_sum(1 + self.z_log_sigma_sq - tf.square(self.z_mean) - tf.exp(self.z_log_sigma_sq), 1)
